...
class PrefControlledDataArguments(DataArguments):
    ...
    preference_config: Dict[float, float] = field(
        ...
        metadata={"help": ("Weight for each preference strength with diff: weight."
...
def control_preference_strength(dataset: DatasetDict, preference_config: dict, max_data_size: int = 2000):
    ...
    for i, b in enumerate(bins):
        ...
        weights_for_each_bin[b] = weights
        all_weights.append(weights)
    sample_per_unit_weight = max_data_size // sum(all_weights)
    ...
    for bin_idx, data_indices in bin_to_data_idx.items():
        weight_for_curr_bin = weights_for_each_bin[bins[bin_idx]]
        samples_for_curr_bin = int(sample_per_unit_weight * weight_for_curr_bin)

        if len(data_indices) > samples_for_curr_bin:
            # randomly sample
            ...
            # just keep all
            print(f"Bin {bin_idx} has {len(data_indices)} samples, less than {samples_for_curr_bin} samples. Using all instead.")
...
def main():
    ...
    raw_datasets['train'] = control_preference_strength(
