...
@dataclass
class NegControlledDataArguments(DataArguments):
    """
    modified from DataArguments to include preference_config and max_data_size
    """

    negative_config: Dict[str, Any] = field(
        default_factory = lambda: {'threshold': 7.0, 'operand': '>='},
        metadata={"help": ("The above will only keep samples with negative score >= 7.0")},
    )
    ...
    def __post_init__(self):
        assert self.negative_config['operand'] in ['>=', '<='], "operand must be either '>=' or '<='"


def control_negative_data_quality(dataset: DatasetDict, negative_config: dict, max_data_size: int = 2000):
    threshold = negative_config['threshold']
    operand = negative_config['operand']
    if operand not in ['>=', '<=']:
        raise ValueError("operand must be either '>=' or '<='")
    
    if operand == '>=':
        subset_dataset = dataset.filter(
            lambda x: x['score_rejected'] >= threshold,
            fn_kwargs={},
            num_proc=8,
            keep_in_memory=True,
            desc=f"Only using data with negative score >= {threshold}",
        )
    else:
        subset_dataset = dataset.filter(
            lambda x: x['score_rejected'] <= threshold,
...
def main():
    parser = H4ArgumentParser((ModelArguments, NegControlledDataArguments, LoggingArguments, DPOConfig))
    ...
    raw_datasets['train'] = control_negative_data_quality(
        dataset=raw_datasets['train'],
        negative_config=data_args.negative_config,
