...
from src.trainers.configs import (
    DataArguments, SelectiveDPOConfig, LoggingArguments, ModelArguments,
    H4ArgumentParser
)
...
from src.trainers.dpo_selective import tokenize_row, DPOSelectiveTrainer
...
def main():
    parser = H4ArgumentParser((ModelArguments, DataArguments, LoggingArguments, SelectiveDPOConfig))
    ...
    column_names.remove("score_chosen")
    column_names.remove("score_rejected")
    ...
    # raw_datasets['train'] = raw_datasets['train'].select(range(512))  # for debugging
    ...
    raw_datasets = raw_datasets.map(
        ...
        keep_in_memory=True,
        desc="Adding full_ids to datasets",
    )
    ...
    if trainer.accelerator.is_main_process:
        ...
        trainer.save_selection_history(force_save=True)
