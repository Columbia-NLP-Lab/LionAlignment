...
from lionalign.evaluation.mt_bench_common import (
    load_questions,
    load_model_answers,
...
def display_pairwise_answer(
    ...
    ### modified
    single_model_1_gamekey = (qid, model_selector1)
    single_model_2_gamekey = (qid, model_selector2)

    judgment_dict = resolve_single_judgment_dict(
        q, model_judgments_normal_single, model_judgments_math_single, multi_turn=False, judge_model=judge_model
    )
    judgment_dict_turn2 = resolve_single_judgment_dict(
        q, model_judgments_normal_single, model_judgments_math_single, multi_turn=True, judge_model=judge_model
    )
    concate_single_score = (
        "##### Model Single Judgment (first turn)\n" +
        "**Model 1**\n" +
        get_single_judge_explanation(single_model_1_gamekey, judgment_dict).split('\n\n')[0] +
        "\n\n**Model 2**\n" +
        get_single_judge_explanation(single_model_2_gamekey, judgment_dict).split('\n\n')[0]
        # + "\n\n##### Model Single Judgment Explanations (first turn)\n" +
        # "**Model 1**\n" +
        # get_single_judge_explanation(single_model_1_gamekey, judgment_dict) +
        # "\n\n**Model 2**\n" +
        # get_single_judge_explanation(single_model_2_gamekey, judgment_dict)
    )

    concate_single_score2 = (
        "##### Model Single Judgment (second turn)\n" +
        "**Model 1**\n" +
        get_single_judge_explanation(single_model_1_gamekey, judgment_dict_turn2).split('\n\n')[0] +
        "\n\n**Model 2**\n" +
        get_single_judge_explanation(single_model_2_gamekey, judgment_dict_turn2).split('\n\n')[0]
        # + "\n\n##### Model Single Judgment Explanations (second turn)\n" +
        # "**Model 1**\n" +
        # get_single_judge_explanation(single_model_1_gamekey, judgment_dict_turn2) +
        # "\n\n**Model 2**\n" +
        # get_single_judge_explanation(single_model_2_gamekey, judgment_dict_turn2)
    )

    return chat_mds + [explanation] + [explanation_turn2] + [concate_single_score] + [concate_single_score2]
...
def build_pairwise_browser_tab():
    ...
    ## modified
    model_single_score = gr.Markdown(elem_id="model_explanation")
    model_single_score2 = gr.Markdown(elem_id="model_explanation")
    ...
    question_selector.change(
        ...
        chat_mds + [model_explanation] + [model_explanation2] + [model_single_score] + [model_single_score2],
    )
