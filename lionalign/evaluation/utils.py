...
from lionalign.constants import OPENAI_MODEL_LIST
import subprocess
import socket
import signal
import time
...
def run_sglang_server(model_path, port, chat_template, result_save_path, tokenizer_path=None):
    # gemma requires chat template to work (which adds in the system message)
    if "gemma" in model_path.lower():
        assert chat_template is not None, "Gemma model requires chat template to work normally. Comment this line only if you know what you are doing."
        pass
    ...
    print("running sglang server with command: ", sglang_command)
