...
class DPOTrainer(Trainer):
    ...
    #         raise ValueError("Trainer: train_dataset should be an instance of a huggingface Dataset class.")

    #     # split dataset across gpus equally
    #     world_size = self.args.world_size
    #     rank = dist.get_rank()

    #     # Function to perform interleaved split
    #     def interleaved_split(dataset, world_size, rank):
    #         num_samples = len(dataset)
    #         indices = list(range(rank, num_samples, world_size))
    #         return dataset.select(indices)

    #     # note that this doesn't give a balanced split
    #     self.train_dataset = interleaved_split(self.train_dataset, world_size, rank)

    #     logger.info(f"\n\nRank {rank} has {len(self.train_dataset)} samples.\n\n")

    #     # locally computing the reference log probabilities
    #     dataloader_params = {
    #         # use the eval batch size for the reference log probs
    ...
    #         padded_batch = move_to_device(padded_batch, self.accelerator.device)
    ...
    #     all_reference_rejected_logps = torch.cat(all_reference_rejected_logps).float().numpy()

    #     gc.collect()
    #     torch.cuda.empty_cache()

    #     lengths = [None] * world_size
    #     dist.all_gather_object(lengths, len(self.train_dataset))
    #     min_length = min(lengths)

    #     #sync the size of the dataset across all gpus
    #     self.train_dataset = self.train_dataset.select(list(range(min_length)))
    #     all_reference_chosen_logps = all_reference_chosen_logps[:min_length]
    #     all_reference_rejected_logps = all_reference_rejected_logps[:min_length]
    ...
    def concatenated_inputs(
        ...
        for k in concatenated_batch:
            if "labels" in k:
                ...
                pad_value = False
            concatenated_batch[k] = pad_sequence(concatenated_batch[k], batch_first=True, padding_value=pad_value).to(
                device=device
            )
    ...
    def _inner_training_loop(
        ...
        # reconstruct the train_dataloader
        train_dataloader = DataLoader(self.train_dataset, **dataloader_params)
