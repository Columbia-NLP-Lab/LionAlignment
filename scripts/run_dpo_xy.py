...
from lionalign.arguments import (
    DataArgumentsXY, LoggingArguments, ModelArguments,
    H4ArgumentParser
)
from lionalign.data.utils import add_full_id, apply_chat_template, get_datasets_xy
...
from lionalign.trainer.efficient_dpo_trainer import tokenize_row, EfficientDPOTrainer
from lionalign.trainer.efficient_dpo_config import EfficientDPOConfig
from lionalign.constants import DPO_DATA_COLUMNS_TO_REMOVE, DPO_DATA_MIX_COLUMNS
...
@dataclass
class SubsampleDataArguments(DataArgumentsXY):
    """
...
def prepare_datasets(
    data_args: SubsampleDataArguments,
...
def maybe_precompute_all_ref_probs(
    ...
    ###### temporary change model_args to ref_model_args
    ori_model = model_args.model_name_or_path
    model_args.model_name_or_path = ref_model
    ref_tokenizer = get_tokenizer(model_args, data_args)
    if 'stablelm' in ref_tokenizer.name_or_path:
        logger.warning("Setting pad token id to 100288 assuming you are using StableLM tokenizer")
        ref_tokenizer.pad_token_id = 100288
    
    #### retokenize since the tokenizer might be different
    retokenized_datasets = raw_datasets.map(
        tokenize_row,
        fn_kwargs={
        ...
        num_proc=data_args.preprocessing_num_workers,
        desc="(Map) Truncating prompt and responses for precompute"
    )
    ...
    if os.path.exists(train_save_path):
        # load csv
        logger.info(f"Loading precomputed train reference log probabilities from {train_save_path}")
        ...
        trainer = trainer or EfficientDPOTrainer(
            model,
            args=training_args,
            train_dataset=retokenized_datasets["train"],
            tokenizer=ref_tokenizer,
        )
        
        train_logps = trainer.compute_all_reference_log_probs(retokenized_datasets['train'])
    ...
    if os.path.exists(test_save_path):
        ...
        test_logps = trainer.compute_all_reference_log_probs(retokenized_datasets['test'])
        test_logps_df = pd.DataFrame(test_logps).drop_duplicates(subset='full_id')
        test_logps_df.to_csv(test_save_path, index=False)

    ### change model back
    model_args.model_name_or_path = ori_model
    
    # update the datasets
    # join the tokenized_datasets (for training) with retokenized_datasets
    train_df: pd.DataFrame = tokenized_datasets['train'].to_pandas()
    ...
    # update the datasets
    test_df: pd.DataFrame = tokenized_datasets['test'].to_pandas()
...
def main():
    global logger
    parser = H4ArgumentParser((ModelArguments, SubsampleDataArguments, LoggingArguments, EfficientDPOConfig))
    ...
    raw_datasets = get_datasets_xy(data_args, splits=data_args.dataset_splits, col_to_mix=column_to_keep)
    logger.info(
    ...
    tokenized_datasets, raw_datasets = prepare_datasets(
        data_args=data_args,
